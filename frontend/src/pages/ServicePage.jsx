...
const ServicePage = () => {
  ...
  const renderService = (service, index) => {
    ...
    return (
      <div key={index} className='flex flex-col flex-wrap w-full justify-center mb-[10vh] max-lg:items-center'>
        <div className={`flex flex-row max-lg:flex-col max-lg:p-2 justify-start items-end px-2 py-4 rounded-lg overflow-hidden bg-zinc-800 dark:bg-zinc-800 max-lg:bg-opacity-95 border-orange-300 lg:w-2/3 max-lg:max-w-[85dvw] border-y-8 ${isLeftDiv ? 'lg:self-start lg:ml-[8dvw]' : 'lg:self-end lg:mr-[8dvw]'}`}>
  ...
  return (
    <div className='overflow-y-auto bg-zinc-100 dark:bg-zinc-800 justify-center items-center'>
      <Navbar />
      <h1 className='text-center text-8xl max-md:text-6xl max-md:mt-12 text-black rounded-2xl md:mb-[15vh] max-md:mb-12 mt-[7vh] dark:text-white font-display font-bold tracking-tight'>Gallery</h1>
      {loading ? (
